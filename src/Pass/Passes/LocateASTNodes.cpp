...
bool locateASTNodes(SpmdTUTy &spmd_tu, clang::ASTContext &ast_context,
    ...
    SPMDFY_INFO("Number of Shmem Nodes Detected: {}",
                workspace.shmem_queue.size());
    return false;
}
...
CFGNODE_DEF_VISITOR(Internal, internal) {
    ...
    SPMDFY_INFO("Visiting InternalNode {} of type {}", internal->getName(),
                node_name);
    if (node_name == "CallExpr") {
        auto call_expr = internal->getInternalNodeAs<const clang::CallExpr>();
        if (call_expr->getDirectCallee()->getNameAsString() ==
            "__syncthreads") {
            SPMDFY_INFO("Detected synthreads after {}",
                        internal->getPrevious()->getName());
    ...
    if (node_name == "Var") {
        ...
        if (var_decl->hasAttr<clang::CUDASharedAttr>()) {
            SPMDFY_INFO("Detected SharedMemory Nodes {}",
                        internal->getPrevious()->getName());
